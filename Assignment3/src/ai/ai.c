...
void find_solution(sokoban_t* init_data, bool show_solution)
{
	 ...
	 * reviewing the the algorithm code example start from line 6 until the end
	 * starts with the while loop which is while priorityQueue != empty do....
	 * */
	...
	while(pq.count != 0){
		...
		for(int a = left; a <= down; a++){
			//as node_t** new_node so create a pointer new_node
			node_t* new_node = NULL;
			bool playerMoved = applyAction(init_data,n,&new_node,a);
			generated_nodes++;
			if(!playerMoved||simple_corner_deadlock(init_data, &(new_node -> state))){
                freeNode(init_data, new_node);
				continue;
			}
			//new_node->这里可以改写
			...
			//void*value,最后这里可以改因为value不重要 改写一下
			ht_insert(&hashTable,flat_map,flat_map);
			heap_push(&pq,new_node);
		}
	}
	for (int i = 0; i < pq.count; i++) {
        freeNode(init_data,pq.heaparr[i]);
    }
    free(pq.heaparr);   
